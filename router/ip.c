...
struct sr_rt* sr_get_longest_prefix(struct sr_rt* rt, uint32_t ip)
{
    ...
    sr_print_routing_entry(rt_match);
    return rt_match;
}
...
void forwarding_packet(struct sr_instance* sr, uint8_t * packet, unsigned int len, char* interface)
{
    ...
    fw_pkt = malloc(len - ETH_HDR_SIZE);
    memcpy(fw_pkt,packet + ETH_HDR_SIZE, len - ETH_HDR_SIZE);

    rt_entry = sr_get_longest_prefix(sr->routing_table,(ip_hdr->ip_dst));
    if (NULL != rt_entry)
    {
        send_ip_packet(sr,fw_pkt, len - ETH_HDR_SIZE, rt_entry->interface, ntohl(rt_entry->gw.s_addr));
    }
